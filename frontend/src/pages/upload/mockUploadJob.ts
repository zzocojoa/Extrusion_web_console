  ...
  const files: UploadJobFile[] = [
    {
      ...
      folderLabel: "PLC",
      folderPath: "training-source/upload-job",
      filename: "training_upload_fixture_A.csv",
      path: "training-source/upload-job/A.csv",
      kind: "plc",
      fileDate: "2026-06-02",
    ...
    },
    {
      ...
      folderLabel: "PLC",
      folderPath: "training-source/upload-job",
      filename: "training_upload_fixture_B.csv",
      path: "training-source/upload-job/B.csv",
      kind: "plc",
      fileDate: "2026-06-02",
...
function buildEvents(jobId: string, startedAtMs: number, status: UploadJobStatus, step: number): JobEvent[] {
  ...
  const events: JobEvent[] = [
    ...
    event(3, jobId, new Date(startedAtMs + 500).toISOString(), "info", "file.started", "Started training upload fixture A.", 1),
  ];
  if (step > 25) events.push(event(4, jobId, new Date(startedAtMs + 1600).toISOString(), "debug", "file.progress", "Progress 5200/16000.", 1));
  if (step > 65) events.push(event(5, jobId, new Date(startedAtMs + 4600).toISOString(), "info", "file.succeeded", "Completed training upload fixture A.", 1));
  if (status === "partial_failed") {
    events.push(event(6, jobId, new Date(startedAtMs + 5100).toISOString(), "info", "file.started", "Started training upload fixture B.", 2));
    events.push(event(7, jobId, new Date(startedAtMs + 6500).toISOString(), "error", "file.failed", "Failed training upload fixture B: Edge upload timed out after retry attempts.", 2));
