...
- Updated the operator launcher shortcut lifecycle: installed shortcuts now use
  ...
  localhost-only health, process id, and the expected uvicorn backend command
  before terminating anything; maintainer stop/restart scripts remain available
  without exposing separate operator shortcuts.
