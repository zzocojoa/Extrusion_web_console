...
owner/fence/claim-and-completion deadlines; an owner-only exact-byte recovery
snapshot with canonical-root/handle identity, ceiling, capacity, confidentiality,
retention, and separately approved early-disposition controls; and one pre-
reserved target-side Delete mutation marker whose atomic transaction and marker-
first reconciliation prove outcome despite response loss or external writers.
...
Docker Desktop's `Expose daemon on tcp://localhost:2375 without TLS` setting must remain off for normal operator use, package validation, and routine release checks. Maintainers may turn it on only temporarily for a bounded diagnostic or failure-reproduction case when sanitized Docker/Vector log evidence is required and cannot be collected through the normal status-class probes; turn it off again as soon as that evidence is captured. Vector or Grafana attention is a non-core observability caveat when Supabase API, DB, Edge, Preview, and Audit evidence are otherwise normal; it must not become a reason to run Docker cleanup/reset or to skip the normal upload gates.
