...
Docker Desktop's `Expose daemon on tcp://localhost:2375 without TLS` setting must remain off for normal operator use and package validation. Maintainers may turn it on only temporarily for developer diagnostics, release validation, or failure reproduction when sanitized Docker/Vector log evidence cannot be collected another way; turn it off again as soon as that evidence is captured. Vector or Grafana attention is a non-core observability caveat when Supabase API, DB, Edge, Preview, and Audit evidence are otherwise normal; it must not become a reason to run Docker cleanup/reset or to skip the normal upload gates.
