...
- `db_query_failed` now has focused reconciler and service coverage distinct from DB unreachable, including persisted run/item status, `db_match_count = NULL` when no DB result was observed, and operator i18n. Additional driver/network failure variants remain appropriate for operator-environment testing.
- The service streams CSV rows and stages deduplicated exact keys in the temporary `preview_key_stage` table before one `all_metrics` join. Very large CSV behavior remains bounded by extraction/reconciliation timeouts and in-memory key deduplication.
- Reconciliation still opens a DB connection and temporary staging table per candidate file to keep file-level memory and failure isolation bounded. A Preview near the 500-file cap can spend substantial time on repeated connection/DDL work; any run-scoped reuse must preserve cancellation, per-file classification, and bounded memory.
