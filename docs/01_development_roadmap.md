...
Implementation in progress on `codex/machine-global-coordinator-foundation`: the first docs/164/171/173 production foundation resolves the fixed Windows machine-authority path through the OS Known Folder API, independently of package, Windows user, config, and `EWC_STATE_DB_PATH`. Its pre-bound fail-closed SQLite coordinator repository requires installer-persisted exact file-binding and monotonic external generation-floor attestations outside the SQLite file, advances that floor before each DB CAS commit so uncertain publication becomes fail-closed, binds response-loss retries to their source generations, prevents blocked-state takeover, and has deterministic thread/process concurrent-claim plus replacement/in-place rollback rejection tests. It deliberately exposes no production open factory, does not auto-provision the authority, provide the production Windows ACL/security-descriptor/file-binding/generation-floor verifier and atomic floor store or installer persistence, expose coordinator release, wire any API or protected stage, or authorize any operational action. V2 readiness remains blocked until those controls and the remaining document contracts are implemented and reviewed.
...
- Still required before any cutover can proceed: implement and deterministically test production atomic content-manifest binding with an immutable snapshot retained through Preview, Start, and Retry so no worker reopens the operational source; make full-byte copying explicit and enforce approved size, protected capacity/confidentiality, retention, a human claim/completed-publication deadline inside inventory validity, and preparation-approved bounded automatic cryptographic disposal; require a separately provisioned/verified owner-only trusted target baseline and compare the first observed authenticated DB identity against it before publishing any exact target binding; implement protected expiring single-use Start/Retry approvals atomically claimed with that snapshot and exactly one job/event, plus a human-bounded unrenewable active-use lease no later than target-binding/snapshot validity, target-side transaction fence and durable outcome marker that prevent false rollback across the remote-commit/local-release window; block and reconcile unknown outcomes, and derive Retry scope from fresh exact DB reconciliation of the entire rolled-back attempted subset; rebuild and verify the exact executing package; only then repeat the read-only inventory, create an execution-adjacent successor record with human-supplied operator-PC and privacy-safe exact-source bindings, separately approve and review protected single-use manifest/snapshot and target-identity preparation records, and request the later separately approved fresh Preview; record any required partial-overlap disposition and final release-owner/operator sign-off; and resolve or explicitly accept the current non-core Grafana/Vector attention with an owner and stop/rollback procedure. The current size/mtime signature, a separate pre-call scan, source-path reopen, hidden full-byte retention, first-observation trust of the configured DB, replayable mutation approval text, and a best-effort action timer are insufficient. Local Supabase start/stop remains separately approved operational evidence only when a cutover actually requires lifecycle control.
- Separately, operational Delete remains blocked: its CSV snapshot is source/key
...
Current transition status: `NO-GO` for cutover execution; `CONDITIONAL GO` candidate after the hard gates above are satisfied. Core Ops request/count behavior, representative legacy CSV automation, synthetic soak, the separately approved operator-PC Preview-only run, later read-only UI/log observations, and the package/inventory baseline record in `docs/182` are complete for their stated scopes. The earlier Preview had zero target-only rows, so Start Upload and Retry Failed were not applicable and were not executed, but it predates `docs/182` and its 64,766 partial-overlap rows still require human disposition if that evidence is used. Production manifest/snapshot binding retained through Preview/Start/Retry, a separately provisioned/verified trusted target baseline, protected target-identity/Start/Retry approval lifecycles, bounded unrenewable leases with target transaction markers/commit-unknown recovery/whole-subset reconciliation, deterministic replacement/concurrency/tamper/replay/source-reopen/wrong-initial-target/lease-disposition/commit-window tests, a rebuilt and verified package, successor inventory, separately approved deadline-bound manifest/snapshot preparation, separately approved read-only target-identity preparation with human review of its exact-match result, later approved current Preview, final sign-off, and Grafana/Vector risk ownership prevent a current proceed decision. Baseline provisioning/rotation is separately controlled and not authorized by these cutover documents.
