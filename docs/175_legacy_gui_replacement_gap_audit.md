...
must a separately controlled pre-provisioned verified target baseline and a pre-
existing human-reviewed protected source-root binding/config generation already
exist; the read-only inventory may only revalidate that binding. Then an
execution-adjacent successor inventory with human-supplied
operator/source bindings, a separately approved protected single-use manifest
preparation that revalidates and snapshot-binds the exact root, separately
approved read-only target-identity preparation with human
exact-match review, and a later separately approved current Preview may be
prepared. This chain does not authorize baseline provisioning/review/repair. Any
...
| Local Supabase status/start/stop | Implemented Core Ops; read-only readiness evidence passed, mutation approval lifecycle not implemented/proven | Runtime start blocks when required containers are missing, starts only stopped allowed containers or `supabase start` after container precheck, and writes runtime audits. Command runner forbids init/reset/create/rm/prune/up/down. | Current operator-PC read-only status reports Docker, WSL, CLI, API, DB, Studio, and Edge ready with no missing required container. Overall remains `attention` only because Grafana is unreachable and Vector is unhealthy. | Any app-controlled start/stop needs the exact expiring single-use `docs/164` control approval/state/action/before-after/operation/outcome/audit lifecycle, an authenticated machine-global coordinator atomically excluding every nonempty canonical consumer—including manifest/target-identity/Preview/Start/Retry/Delete-preflight/Delete/disposition/restore/reconcile/recovery/final-signoff—in both claim orders, and deterministic partial-transition/replay/concurrency/response-loss tests; non-core attention needs explicit disposition. | Medium: core runtime is ready, but a status observation cannot authorize lifecycle mutation; sequential container control can partially transition before failure; and non-core attention must not be confused. | Implement/test the shared coordinator plus durable `commit_pending`/`commit_unknown_blocked` recovery before crediting start/stop evidence; accept or resolve Grafana/Vector caveat; keep reset/cleanup prohibited. | `backend/app/services/runtime_control.py`; `backend/app/services/command_runner.py`; `README.md`; `docs/164_operator_data_mutation_safety_gate.md`; `docs/167_v2_observability_hardening_evidence.md` |
...
| Settings save | Implemented Core Ops behavior; operational approval lifecycle not implemented/proven | `GET /api/config` and `PUT /api/config` exist; save validates payload, blocks env-overridden keys, writes config atomically, clears settings cache, and records `settings.save` audit rows. | Roadmap marks config API and Settings save UI done, while `docs/164` separately requires an exact expiring single-use control approval/state, config-generation/change-set binding, atomic claim with the shared machine-global operation coordinator, durable operation outcome, reverse exclusion against every protected stage, response-loss idempotency, and deterministic negative tests. | Implement/test that protected lifecycle before operational save evidence; read-only `GET /api/config` remains allowed and is not approval. | Medium: wrong, stale, replayed, or concurrently substituted settings can bind the wrong source/target, and a save during manifest/target-identity/upload/delete preparation can invalidate the active chain. | Preserve the read-only snapshot; do not credit a Settings mutation until the canonical lifecycle/tests and separate approval pass. | `backend/app/api/config.py`; `backend/app/services/config_service.py`; `docs/164_operator_data_mutation_safety_gate.md`; `docs/01_development_roadmap.md` |
| Already-in-DB exact-key delete | Implemented backend/API/UI path, not operational/V2 proof | Backend/UI enforce selected `already_in_db` scope, but current code lacks the future target-global coordinator, fenced preflight owner/deadline/result CAS, canonical-root exact-byte source-provenance snapshot, DELETE side-effect/recoverability binding, atomic complete-row DB before-image and protected restore/disposition approvals, distinct approval-owned source/DB/restore cleanup lifecycles, target mutation marker/marker-first reconcile, and privacy-safe opaque binding boundary. Current CSV/key evidence cannot recreate stored DB values that differ. | v1 scope includes this only when separately approved and guarded, but current code cannot satisfy the full `docs/164`/`docs/171` contract. | Implement/test every coordinator, preflight, source-provenance snapshot, side-effect binding, atomic DB-before-image/restore/disposition, non-interchangeable source/DB/restore cleanup ids/deadlines/states/evidence, committed dual-record and aborted source-only close, single-use Delete run, marker, exact-target, and privacy requirement in `docs/164`/`docs/171`. | Production-critical: wrong/duplicate/concurrent mutation, false outcome inference, loss of exact pre-delete values or side effects, arbitrary-source capture, cross-record cleanup, or disclosure/recovery of exact keys/rows. | Keep operational Delete blocked; the current v1 path cannot count as operational or V2 proof. | `backend/app/services/upload_delete.py`; `frontend/src/pages/UploadPage.tsx`; `docs/164_operator_data_mutation_safety_gate.md`; `docs/171_v2_operational_delete_verification_gate.md` |
...
| Delete expansion | Deferred V2 feature | Feature gate is `implemented=False`; no broader delete policy is executable. Current code exposes selected `already_in_db` exact-key compatibility behavior only; it is not operational readiness proof. | `docs/170` now defers to the protected `docs/164`/`docs/171` lifecycle and `docs/173` cross-action coordination; any expansion also needs explicit policy and fixture proof. | Full current protected Delete contract, policy limits, fixture DB evidence, production approval format, reconcile/audit/exact DB-before-image recovery proof. | Production-critical if broadened without proof. | Keep out of v1 cutover; treat legacy endpoints as compatibility context and prepare future fixture-first design only. | `backend/app/services/config_service.py`; `docs/164_operator_data_mutation_safety_gate.md`; `docs/170_v2_delete_expansion_fixture_gate.md`; `docs/171_v2_operational_delete_verification_gate.md`; `docs/173_v2_operational_upload_verification_gate.md`; `docs/165_v2_status_matrix.md` |
| Operational DB delete verification | Deferred pending full hardened contract | Code path exists, but no operational DB mutation was run and current code lacks the target-global coordinator, fenced preflight owner/deadline/result lifecycle, source-provenance snapshot, DELETE side-effect/recoverability binding, atomic exact DB-before-image/restore/disposition lifecycle, distinct approval-owned cleanup lifecycles, single-use Delete approval/run, and target marker. | `docs/171` does not approve execution; `docs/164` makes every hardened lifecycle mandatory. | Implement/test the complete `docs/164`/`docs/171` coordinator/preflight/source-provenance/side-effect/DB-before-image/restore/disposition/Delete-marker contract, including non-interchangeable source-snapshot, Delete-created DB-before-image, and restore-owned cleanup ids, with separate committed and aborted close semantics, then obtain separate current evidence and approval. | Production-critical. | Keep blocked; current v1 behavior cannot count as operational/V2 proof or be bundled with cutover. | `docs/171_v2_operational_delete_verification_gate.md`; `docs/164_operator_data_mutation_safety_gate.md` |
| Operational upload verification | Partial: historical Preview-only behavior passed; new Preview blocked | Preview/Start/Retry code exists with approval-count gates. One separately approved operator-PC Preview-only run succeeded with DB reachable and `target=0`; Start Upload remained disabled, so Start Upload and Retry Failed were not applicable and were not executed. Current workers do not prove they retain Preview-verified bytes, bind the source to a pre-existing reviewed exact root/config generation, bind the DB to a pre-existing trusted expected identity, or safely resolve the remote-commit/local-release window. | `docs/173` remains the evidence chain and `docs/164` remains the approval-wording source; both block a new Preview until protected source-root/manifest/snapshot binding, separately verified target trust anchor plus target-identity preparation, and the Start/Retry target-outcome protocol exist. `docs/182` supplies baseline evidence only. | Production atomic manifest/snapshot binding, pre-provisioned reviewed source-root and verified owner-only target baselines, inventory no-write source revalidation, separately approved manifest root-state transition and target-identity exact-match preparation/review, target-fenced durable mutation outcomes, commit-unknown recovery, whole-attempt Retry reconciliation, deterministic replacement/concurrency/tamper/replay/source-reopen/wrong-initial-target/lease-disposition/commit-window tests, new package evidence, successor inventory, later approved Preview, partial-overlap disposition, and final sign-off remain. | Production-critical until the protected source/target/outcome lifecycles exist: actions can bind different roots/bytes or a wrong same-port DB, reuse records, race disposition, falsely certify rollback, or omit rolled-back rows. | Implement/test the protected lifecycle first; then ensure separately controlled reviewed source-root and verified target baselines exist, rebuild/verify, let inventory only revalidate source binding/config generation, separately prepare manifest/snapshot through the exact root transition, separately approve/read-only-prepare and review the target identity, and only afterward seek Preview approval. This chain does not authorize either baseline's provisioning/review/repair/rotation. Preserve the earlier result only as historical `no target-only upload` evidence. | `backend/app/services/upload_preview.py`; `backend/app/api/upload_jobs.py`; `docs/173_v2_operational_upload_verification_gate.md`; `docs/165_v2_status_matrix.md`; `docs/176_v1_cutover_go_no_go_validation_plan.md` |
...
| Legacy CSV fixture and soak validation | Completed for V2 WP-01 Preview-only/observation scope | Current main covers representative UTF-8/CP949 PLC, temperature, and integrated fixtures, exact reconciliation and DB-status semantics, a deterministic 25,000-row synthetic soak, and one approved large real `folder_all` Preview. | Roadmap and `docs/11` record the automated foundation; WP-02 consolidates the operator evidence and leaves final cutover sign-off separate. | Preserve fixture/soak coverage. Every new execution chain requires a fresh Preview and full `docs/164`/`docs/173` admission; re-enter it on any inventory/artifact/approval/manifest/snapshot/trusted-target/target-binding change, expiry, revocation, or generation drift. | Low-medium: unrepresented future CSV shapes or stale-chain reuse can regress. | Add fixtures when a new legacy shape is discovered; never reuse an operational Preview or use mutation as a fixture substitute. | `tests/backend/fixtures`; `tests/backend/test_upload_preview_large_synthetic_soak.py`; `tests/backend/test_upload_preview_reconciliation.py`; `docs/11_upload_preview_large_csv_soak.md`; `docs/01_development_roadmap.md` |
| README/API live-smoke omission and schema consistency | Documentation safety correction complete; zero-boundary tests remain a mandatory implementation blocker | Backend requires `expectedTargetRows` for Upload Job start and validates optional `expectedTargetFiles`; README intentionally omits live Preview/Upload Job POST examples because the current runtime does not enforce the protected lifecycle required for operational use. | API source rejects missing/non-positive expected target rows; current tests cover missing and positive-count mismatch paths. The required future cases are `expectedTargetRows: 0` -> 422 `expected_target_rows_required` and optional `expectedTargetFiles: 0` -> 422 `expected_target_files_invalid`, each with a sanitized blocked audit and zero job creation. | Add those deterministic boundary tests before cutover credit; do not reintroduce an ad hoc live POST recipe. | Medium: a zero-value contract regression can create an unintended job or make documentation overstate test coverage. | Keep live POST examples absent; implement both zero-boundary tests and verify source/schema/API behavior synthetically. | `backend/app/api/upload_jobs.py`; `backend/app/schemas/upload_jobs.py`; `tests/backend/test_upload_jobs_api_contract.py`; `docs/03-analysis/start-upload-expected-count-contract.analysis.md`; `README.md` |
...
- Preview execution behavior passed for the observed operator-PC source: DB
  ...
  baseline and pre-existing human-reviewed protected source-root binding/config
  generation must already exist. Inventory may only revalidate the source
  binding. Only then may successor inventory, separately approved protected
  manifest/snapshot preparation that snapshot-binds the exact root, separately approved read-
  ...
  baseline provisioning, review, rotation, or repair.
...
- Current cutover execution is `NO-GO`. A `CONDITIONAL GO` candidate requires
  ...
  content-addressed ZIP/installer plus authenticated full-file manifest and
  verified installed/executing trees, an
  already provisioned/verified trusted target baseline and pre-existing reviewed
  source-root binding/config generation,
  ...
  immutable authenticated, generation-bound, non-revoked release-owner/operator
  sign-off,
...
- README intentionally omits live Upload Preview and Upload Job POST examples
  ...
  mismatch paths. Before cutover credit, add deterministic
  `expectedTargetRows: 0` and optional `expectedTargetFiles: 0` regressions with
  the exact 422 reason, one sanitized blocked audit, and zero job creation.
...
- Any operational Start Upload still needs same-snapshot runtime enforcement, an
  ...
  approval after fresh Preview evidence. The target-global coordinator must use
  one fixed authenticated machine-global authority across Windows users,
  installations, packages, and state-DB paths, and every target mutation must
  enforce its target-side mutation epoch.
- Retry Failed needs the same lease/outcome enforcement, authoritative rollback
  ...
  record whose claim deadline is pre-reserved by the source action approval and
  whose superseded predecessor terminates as `superseded_disposed`, a separate approval
...
- Any operational DB delete remains blocked until a protected expiring preflight
  ...
  schema/column/side-effect/ceiling bindings while an engine-appropriate DDL fence
  proves every DELETE/restore-INSERT secondary relation and external effect is
  absent or inert, committed dual-record and aborted
  source-only cleanup have the correct separate authority, exact restore and every
  record that exists have separately approved disposition, a separate protected expiring Delete approval is atomically
  claimed with that result and one run, a human-bound non-renewable DELETE active-
  use commit deadline/positive reconcile margin plus shared target-side epoch
  fence prevents delayed commit after reconcile/cleanup, and a pre-reserved target marker proves
  the atomic Delete outcome by marker-first reconciliation. An unresolved marker
  at the standard retention deadline requires its pre-authorized bounded incident
  escrow; it may never infer abort or release the coordinator, and unknown-at-
  reconcile-cutoff immediately claims cleanup for strict completion before the
  cleanup deadline with permanent recovery-loss/security-incident NO-GO. Exact-target and
...
| --- | --- | --- | --- | --- | --- |
| A later cutover reuses any prior Preview chain. | High | Medium | Every new execution chain re-enters the complete `docs/164`/`docs/173` sequence; stop on any inventory/artifact/approval/manifest/snapshot/trusted-target/target-binding change, expiry, revocation, generation drift, or Preview mismatch. | Operator approval plus maintainer evidence capture. | Current release trust/artifact metadata, fresh inventory/manifest/target binding/Preview, Start evidence only if target rows exist, Retry evidence only if needed. |
...
| Destructive DB delete or its prerequisite preflight is run with replayable approval text or CSV-only rollback evidence. | Production-critical | Low if gates are followed | Keep operational delete blocked until a protected expiring single-use preflight approval is atomically bound to exactly one preflight, its ready result is single-consumer, a separate protected expiring single-use delete approval is atomically claimed with that result and exactly one delete run, and the target transaction holds an engine-appropriate DDL/schema fence, proves every DELETE/restore-INSERT secondary effect absent/inert, and preserves a complete typed DB before-image atomically with DELETE/marker; use `docs/171` as evidence input only. | Backend lifecycle implementation plus separate operator preflight/destructive/restore/disposition approvals. | Deterministic replay/concurrency tests, preflight and delete bindings, exact key/row/column counts, source-vs-DB value-difference and concurrent DDL/cascade/trigger/CDC cases, before-image/marker atomicity, committed dual-record versus aborted source-only cleanup, split expiry authority, opaque protected ids with owner-only row values, exact restore, and post-run disposition evidence. |
...
- Status: blocked for any new Preview. The next work is a protected production
  ...
  separately controlled verified target baseline and pre-existing human-reviewed
  source-root binding/config generation be confirmed, inventory read-only
  revalidate that binding, and protected manifest/snapshot preparation be separately approved
  to revalidate/snapshot-bind the exact root,
  ...
  This work package does not authorize baseline provisioning/review/rotation/repair.
...
- Prepare a separate package for selected `already_in_db` exact-key delete only
  ...
  before-image/marker atomic transaction, DELETE side-effect/recoverability
  binding, exact restore, and committed dual-record versus aborted source-only
