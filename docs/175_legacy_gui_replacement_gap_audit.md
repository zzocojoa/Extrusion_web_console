...
| Already-in-DB exact-key delete | Implemented backend/API/UI path, not operational/V2 proof | Backend/UI enforce selected `already_in_db` scope, but current code lacks the future target-global coordinator, fenced preflight owner/deadline/result CAS, canonical-root exact-byte recovery snapshot and protected disposition approval, target mutation marker/marker-first reconcile, and privacy-safe opaque binding boundary. | v1 scope includes this only when separately approved and guarded, but current code cannot satisfy the full `docs/164`/`docs/171` contract. | Implement/test every coordinator, preflight, recovery snapshot/disposition, single-use Delete run, marker, exact-target, and privacy requirement in `docs/164`/`docs/171`. | Production-critical: wrong/duplicate/concurrent mutation, false outcome inference, loss of recovery bytes, arbitrary-source capture, or disclosure/recovery of exact keys. | Keep operational Delete blocked; the current v1 path cannot count as operational or V2 proof. | `backend/app/services/upload_delete.py`; `frontend/src/pages/UploadPage.tsx`; `docs/164_operator_data_mutation_safety_gate.md`; `docs/171_v2_operational_delete_verification_gate.md` |
...
| Operational DB delete verification | Deferred pending full hardened contract | Code path exists, but no operational DB mutation was run and current code lacks the target-global coordinator, fenced preflight owner/deadline/result lifecycle, immutable exact-byte recovery snapshot/disposition approval, single-use Delete approval/run, and target marker. | `docs/171` does not approve execution; `docs/164` makes every hardened lifecycle mandatory. | Implement/test the complete `docs/164`/`docs/171` coordinator/preflight/recovery-snapshot/disposition/Delete-marker contract, then obtain separate current evidence and approval. | Production-critical. | Keep blocked; current v1 behavior cannot count as operational/V2 proof or be bundled with cutover. | `docs/171_v2_operational_delete_verification_gate.md`; `docs/164_operator_data_mutation_safety_gate.md` |
...
- Any operational DB delete remains blocked until a protected expiring preflight
  ...
  action chains, an owner-only canonical-root exact-byte recovery snapshot has
  bounded capacity/confidentiality/retention plus separately approved early
  disposition, a separate protected expiring Delete approval is atomically
...
| Destructive DB delete or its prerequisite preflight is run with replayable approval text. | Production-critical | Low if gates are followed | Keep operational delete blocked until a protected expiring single-use preflight approval is atomically bound to exactly one preflight, its ready result is single-consumer, and a separate protected expiring single-use delete approval is atomically claimed with that result and exactly one delete run; use `docs/171` as evidence input only. | Backend lifecycle implementation plus separate operator preflight/destructive approvals. | Deterministic replay/concurrency tests, preflight-approval/preflight binding, preflight-result consumer binding, delete-approval/run binding, exact key count, opaque protected binding ids with owner-only integrity material, rollback readiness, post-run evidence. |
...
- Prepare a separate package for selected `already_in_db` exact-key delete only
  ...
  delete run; then use `docs/171` as evidence input under a separate plan.
