...
| Settings save | Implemented Core Ops behavior; operational approval lifecycle not implemented/proven | `GET /api/config` and `PUT /api/config` exist; save validates payload, blocks env-overridden keys, writes config atomically, clears settings cache, and records `settings.save` audit rows. | Roadmap marks config API and Settings save UI done, while `docs/164` separately requires an exact expiring single-use control approval, config-generation/change-set binding, atomic claim, response-loss idempotency, and deterministic negative tests. | Implement/test that protected lifecycle before operational save evidence; read-only `GET /api/config` remains allowed and is not approval. | Medium: wrong, stale, replayed, or concurrently substituted settings can bind the wrong source/target. | Preserve the read-only snapshot; do not credit a Settings mutation until the canonical lifecycle/tests and separate approval pass. | `backend/app/api/config.py`; `backend/app/services/config_service.py`; `docs/164_operator_data_mutation_safety_gate.md`; `docs/01_development_roadmap.md` |
| Already-in-DB exact-key delete | Implemented backend/API/UI path, not operational/V2 proof | Backend/UI enforce selected `already_in_db` scope, but current code lacks the future target-global coordinator, fenced preflight owner/deadline/result CAS, canonical-root exact-byte source-provenance snapshot, DELETE side-effect/recoverability binding, atomic complete-row DB before-image and protected restore/disposition approvals, target mutation marker/marker-first reconcile, and privacy-safe opaque binding boundary. Current CSV/key evidence cannot recreate stored DB values that differ. | v1 scope includes this only when separately approved and guarded, but current code cannot satisfy the full `docs/164`/`docs/171` contract. | Implement/test every coordinator, preflight, source-provenance snapshot, side-effect binding, atomic DB-before-image/restore/disposition, committed dual-record and aborted source-only close, single-use Delete run, marker, exact-target, and privacy requirement in `docs/164`/`docs/171`. | Production-critical: wrong/duplicate/concurrent mutation, false outcome inference, loss of exact pre-delete values or side effects, arbitrary-source capture, or disclosure/recovery of exact keys/rows. | Keep operational Delete blocked; the current v1 path cannot count as operational or V2 proof. | `backend/app/services/upload_delete.py`; `frontend/src/pages/UploadPage.tsx`; `docs/164_operator_data_mutation_safety_gate.md`; `docs/171_v2_operational_delete_verification_gate.md` |
...
| Operational DB delete verification | Deferred pending full hardened contract | Code path exists, but no operational DB mutation was run and current code lacks the target-global coordinator, fenced preflight owner/deadline/result lifecycle, source-provenance snapshot, DELETE side-effect/recoverability binding, atomic exact DB-before-image/restore/disposition lifecycle, single-use Delete approval/run, and target marker. | `docs/171` does not approve execution; `docs/164` makes every hardened lifecycle mandatory. | Implement/test the complete `docs/164`/`docs/171` coordinator/preflight/source-provenance/side-effect/DB-before-image/restore/disposition/Delete-marker contract with separate committed and aborted close semantics, then obtain separate current evidence and approval. | Production-critical. | Keep blocked; current v1 behavior cannot count as operational/V2 proof or be bundled with cutover. | `docs/171_v2_operational_delete_verification_gate.md`; `docs/164_operator_data_mutation_safety_gate.md` |
...
- Any operational DB delete remains blocked until a protected expiring preflight
  ...
  escrow; it may never infer abort or release the coordinator, and final expiry
  is a permanent recovery-loss/security-incident NO-GO. Exact-target and
