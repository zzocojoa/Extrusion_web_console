...
now records a later package-bound inventory baseline. Final cutover still needs
exact executing-package verification, an execution-adjacent successor inventory
record with human-supplied operator/source bindings, a separately approved
current Preview, any required human disposition of partial-overlap rows,
release-owner/operator sign-off, and explicit ownership of the non-core
Grafana/Vector attention state. `docs/182` does not retroactively validate the
earlier Preview.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Upload Preview | Implemented Core Ops; operator Preview behavior passed | `POST /api/upload/preview`, latest/detail/cancel routes exist; results are persisted and exact DB reconciliation is supported when DB URL is configured. | V2 WP-01 covers DB-status correctness, legacy fixtures, and a deterministic 25,000-row soak. Separately approved operator-PC run `prv_ca38650a9e7d` completed `folder_all` as `succeeded/reachable` with 12 files, `target=0`, `risky=0`, and 64,766 partial-overlap rows excluded from target-only upload. `docs/182` records a later package-bound inventory baseline. | Future Preview or mutation decisions require exact executing-package evidence and an execution-adjacent successor inventory record with operator/source binding; the earlier run does not approve Start Upload, and `docs/182` does not validate it retroactively. Human disposition of partial-overlap rows is still required if that run is used for cutover. | Medium: stale binding or hidden partial overlap can produce a wrong readiness decision. | Preserve the earlier run as Preview-only evidence, not read-only evidence; use `docs/182` only as baseline for a separately approved future Preview chain and disposition partial overlap before cutover. | `backend/app/api/upload_preview.py`; `tests/backend/test_upload_preview_reconciliation.py`; `tests/backend/test_upload_preview_large_synthetic_soak.py`; `docs/01_development_roadmap.md`; `README.md` |
...
| Operational upload verification | Partial: Preview-only behavior passed | Preview/Start/Retry code exists with approval-count gates. One separately approved operator-PC Preview-only run succeeded with DB reachable and `target=0`; Start Upload remained disabled, so Start Upload and Retry Failed were not applicable and were not executed. The run also reported 64,766 partial-overlap rows excluded from target-only upload. | `docs/173` remains the four-step evidence chain and `docs/164` remains the approval wording. `docs/182` supplies later package/inventory baseline evidence, not execution-adjacent approval evidence. `docs/165` classifies this item as Partial. | Exact executing-package verification, a successor inventory record with human-supplied operator/source bindings, a separately approved current Preview, any required partial-overlap disposition, and final sign-off remain. Any future nonzero target requires fresh Start Upload approval. | Medium: this proves Preview behavior, not complete cutover evidence or a new data-mutation path. | Use the earlier result as `no target-only upload` evidence only; do not imply all local rows are DB-matched or create mutation work merely to obtain test evidence. | `backend/app/api/upload_jobs.py`; `docs/173_v2_operational_upload_verification_gate.md`; `docs/165_v2_status_matrix.md`; `docs/176_v1_cutover_go_no_go_validation_plan.md` |
...
- Preview execution behavior passed for the observed operator-PC source: DB
  ...
  only. A separately approved future Preview chain requires an
  execution-adjacent successor inventory record with operator/source bindings.
...
- Current cutover execution is `NO-GO`. A `CONDITIONAL GO` candidate requires
  exact executing-package verification, an execution-adjacent successor
  inventory record with human-supplied safe operator-PC and privacy-safe exact
  source bindings, a separately approved current Preview, any required human
...
- Status: partial, sufficient only for a future `CONDITIONAL GO` candidate. Sanitized
  ...
  and Core Ops routes are captured. `docs/182` records a later package-bound
  inventory baseline. Exact executing-package verification, human-supplied
  operator/source bindings, an execution-adjacent successor inventory record, a
  separately approved current Preview, final human sign-off, and non-core caveat
