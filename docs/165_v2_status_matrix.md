...
| ---: | --- | --- | --- | --- |
| 1 | Operational upload verification | `Partial` | One separately approved post-merge `folder_all` Preview-only run succeeded on the operator PC with DB reachable, 12 files, `target=0`, `risky=0`, 64,766 partial-overlap rows excluded from target-only upload, matching Audit evidence, and Start Upload disabled. `docs/182` binds a later package artifact and read-only inventory baseline. `docs/173` remains the evidence-chain source and `docs/164` remains the approval-wording source. | Start Upload is not applicable to the observed zero-target-only run. A future Preview is blocked until production atomic content-manifest/snapshot binding retained through Preview/Start/Retry, a separately verified owner-only trusted target baseline prevents first-observation trust of a wrong same-port DB, one fixed authenticated machine-global coordinator authority excludes every Windows user/package/install/state-DB sibling, deterministic replacement/concurrency/tamper/replay/source-reopen/wrong-initial-target tests pass, and a content-addressed package plus installed/executing-tree evidence is verified. Start/Retry additionally require a human-bounded unrenewable lease, target-side transaction fence/durable outcome marker and mutation epoch, transaction-time exact-absence/no-overwrite protection, exact target-binding validity enforcement, commit-unknown recovery, one owner-token/deadline-fenced source-action reconciliation-creation entitlement whose claim deadline is pre-reserved by its source approval, terminal superseded-record disposal, encrypted/disposed private subsets, and lease/disposition/commit-window tests. Only then may a successor inventory, separately approved deadline-bound protected manifest/snapshot preparation, separately approved read-only target-identity preparation and human exact-match review, and later exact Preview approval be prepared; baseline provisioning/rotation remains separately controlled and unapproved, and partial-overlap disposition plus immutable authenticated final cutover sign-off remain pending. |
...
| 5 | Operational DB delete verification | `Deferred` | `docs/171` defines approval/evidence content, but the current v1 path lacks the mandatory fixed machine-global target coordinator and target-side mutation epoch, fenced preflight owner/deadline/result lifecycle, exact-byte source-provenance snapshot, DDL-fenced direct-row-only DELETE/restore side-effect binding, atomic complete-row DB before-image and disposition/restore/unknown-outcome-escrow approvals, target mutation marker, and safe opaque binding contract. The current CSV-based `rollbackReadiness` cannot prove exact rollback when DB values differ. `docs/164` therefore keeps execution blocked. | Implement/test every `docs/164`/`docs/171` gate: singleton exact-target coordination across Windows users/packages/installations/state DBs; atomic preflight claim/fence/completion CAS; owner-only canonical-root source-provenance snapshot; protected complete typed DB before-image captured atomically with DELETE and marker under exact schema/column/side-effect/ceiling/capacity/confidentiality/retention bindings; bounded encrypted incident escrow for an outcome still unknown at the standard deadline; engine-appropriate DDL/schema fencing that proves all DELETE and restore-INSERT secondary effects absent/inert; committed dual-record versus aborted source-only cleanup with split approval authority; exact restore/disposition; separate single-use Delete approval/run; marker-first reconcile; opaque public ids/private exact material; and deterministic concurrency/crash/privacy/tamper/disposal tests. Current v1 execution cannot count as operational or V2 proof. |
...
| Upload readiness hardening | `Partial: readiness checks complete; approval and immutable-snapshot lifecycles blocked` | `main` includes Start/Retry readiness hardening for local DB target class, Supabase API/DB/Edge readiness, Edge auth key class, and target-only row approval counts. | Does not approve Start Upload or Retry Failed. Future execution is blocked until protected expiring single-use approvals atomically claim the same immutable Preview snapshot with exactly one corresponding job/event, one unrenewable lease, and the approval's pre-reserved target-fenced mutation id; one fixed authenticated machine-global coordinator excludes cross-user/install/state-DB siblings and every target transaction enforces its mutation epoch; all action writes and a durable outcome marker share the authoritative target transaction, while the exact approved-absent keyset is serializably revalidated and conflict-rejecting inserts prevent overwrite; unknown outcomes block/reconcile; each rolled-back source action can publish only one owner-token/deadline-fenced protected whole-attempt DB-reconciliation record using its parent-approved claim deadline, Retry atomically consumes it, superseded records terminate with verified disposal, its private subset is encrypted and disposed, and deterministic target-drift/commit-window/disposition/replay tests pass. |
| API-mode operator package validation | `Completed for historical smoke only` | Merged `docs/166` records API-mode build, package assembly, zip/SHA-256 handoff metadata, launcher/shortcut `-CheckOnly`, and read-only HTTP smoke. Static metadata in that document is historical sample evidence only. | Final GO requires a current content-addressed ZIP/installer, authenticated full-file manifest, installed/executing-tree verification, and immutable authenticated non-revoked sign-off. `package-build-info.json`, label, or source commit alone is insufficient. |
...
| Operational DB delete verification | `Deferred` | `docs/171` defines the full future contract, but current code does not implement its fixed machine-global target coordinator/target-side epoch, preflight owner/fence/deadline CAS, source-provenance snapshot, DDL-fenced direct-row-only DELETE/restore side-effect binding, atomic complete-row DB-before-image/restore/disposition/unresolved-outcome-escrow lifecycle, target mutation marker, or safe opaque-binding lifecycle. | Keep blocked until every `docs/164`/`docs/171` coordinator/preflight/result/source-provenance/schema-fence/side-effect/before-image/restore/disposition/incident-escrow/Delete-run/marker requirement is implemented and deterministically tested, including absent/inert secondary effects, committed dual-record versus aborted source-only cleanup, split expiry authority, marker-first reconciliation, permanent final-escrow incident handling, and owner-only exact material. Current v1 execution cannot count as operational or V2 evidence; later execution still requires a separate plan and approval. |
