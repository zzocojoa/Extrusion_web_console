...
| 5 | Operational DB delete verification | `Deferred` | `docs/171` defines approval/evidence content, but the current v1 path lacks the mandatory target-global coordinator, fenced preflight owner/deadline/result lifecycle, exact-byte source-provenance snapshot, atomic complete-row DB before-image and disposition/restore approvals, target mutation marker, and safe opaque binding contract. The current CSV-based `rollbackReadiness` cannot prove exact rollback when DB values differ. `docs/164` therefore keeps execution blocked. | Implement/test every `docs/164`/`docs/171` gate: singleton exact-target coordination across Preview chains; atomic preflight claim/fence/completion CAS; owner-only canonical-root source-provenance snapshot; protected complete typed DB before-image captured atomically with DELETE and marker under exact schema/column/ceiling/capacity/confidentiality/retention bindings; exact restore and dual-record disposition; separate single-use Delete approval/run; marker-first reconcile; opaque public ids/private exact material; and deterministic concurrency/crash/privacy/tamper/disposal tests. Current v1 execution cannot count as operational or V2 proof. |
...
| Operational DB delete verification | `Deferred` | `docs/171` defines the full future contract, but current code does not implement its target-global coordinator, preflight owner/fence/deadline CAS, source-provenance snapshot, atomic complete-row DB-before-image/restore/disposition lifecycle, target mutation marker, or safe opaque-binding lifecycle. | Keep blocked until every `docs/164`/`docs/171` coordinator/preflight/result/source-provenance/before-image/restore/disposition/Delete-run/marker requirement is implemented and deterministically tested, with marker-first reconciliation and owner-only exact material. Current v1 execution cannot count as operational or V2 evidence; later execution still requires a separate plan and approval. |
