...
| Operational DB delete verification | `Deferred` | `docs/171` defines the full future contract, but current code does not implement its target-global coordinator, preflight owner/fence/deadline CAS, source-provenance snapshot, DDL-fenced direct-row-only DELETE/restore side-effect binding, atomic complete-row DB-before-image/restore/disposition lifecycle, target mutation marker, or safe opaque-binding lifecycle. | Keep blocked until every `docs/164`/`docs/171` coordinator/preflight/result/source-provenance/schema-fence/side-effect/before-image/restore/disposition/Delete-run/marker requirement is implemented and deterministically tested, including absent/inert secondary effects, committed dual-record versus aborted source-only cleanup, split expiry authority, marker-first reconciliation, and owner-only exact material. Current v1 execution cannot count as operational or V2 evidence; later execution still requires a separate plan and approval. |
